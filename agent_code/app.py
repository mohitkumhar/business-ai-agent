...
def auth_signup():
    ...
    try:
        ...
        cur.execute(
            "INSERT INTO businesses (business_id, business_name, industry_type, owner_name) VALUES (%s, %s, %s, %s)",
        ...
        cur.execute(
            "INSERT INTO users (business_id, name, email, password_hash) VALUES (%s, %s, %s, %s) RETURNING user_id",
...
def auth_login():
    ...
    try:
        ...
        cur.execute(
            "SELECT user_id, business_id, name, password_hash FROM users WHERE email = %s",
            (email,),
        )
        user = cur.fetchone()
...
AGENT_REQUEST_COUNT = Counter(
    "agent_requests_total", "Total requests", ["method", "endpoint", "status"]
)
AGENT_REQUEST_LATENCY = Histogram(
    "agent_request_duration_seconds", "Request latency", ["method", "endpoint"]
)
AGENT_INTENT_COUNT = Counter(
    "agent_intent_detections_total", "Intent detections", ["intent"]
)
...
if not GROQ_API_KEY:
    logger.warning(
        "GROQ_API_KEY is not set. Groq-powered responses will fail until it is configured."
    )
...
def _download_whatsapp_media(media_id: str) -> tuple[bytes, str]:
    ...
    try:
        ...
        blob = requests.get(
            url,
            headers={"Authorization": f"Bearer {WHATSAPP_ACCESS_TOKEN}"},
            timeout=(5, 60),  # longer read timeout — media download can be slow
        )
    ...
    try:
        with conn.cursor() as cur:
            ...
            """,
                (
                    business_id,
                    normalized.get("date", date.today()),
                    normalized["type"],
                    normalized["category"],
                    normalized["amount"],
                    f"Bill from {normalized.get('vendor', 'Unknown')}",
                ),
            )
...
def home():
    return jsonify(
        {"status": "healthy", "service": "ProfitPilot Backend", "version": "1.0.0"}
    )
...
def onboarding():
    ...
    try:
        ...
        cur.execute(
            "INSERT INTO users (business_id, name, email, password_hash) VALUES (%s, %s, %s, %s)",
...
def telegram_webhook():
    try:
        ...
        has_attachment = bool(
            message.get("photo") or message.get("document") or message.get("voice")
        )
        ...
        if not text:
            reply = (
                ...
                if has_attachment
                else "Please send a text question so I can help."
            )
        ...
        try:
            ...
            if chat_id is not None:
                _send_telegram_text(
                    chat_id, "Sorry, I could not process that Telegram update."
                )
        except Exception:
            pass
        return internal_error_response(e)
...
def import_transactions():
    ...
    try:
        ...
        try:
            ...
            conn.commit()
            return jsonify(
                {"message": f"Successfully imported {len(rows)} transactions!"}
            ), 201
        finally:
            conn.close()
    except Exception as e:
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    "SELECT 1 FROM daily_transactions WHERE business_id = %s AND description LIKE %s LIMIT 1",
    ...
    try:
        ...
        def get_metrics(s, e):
            ...
            alerts = execute_read_query_params(
                "SELECT COUNT(*) FROM alerts WHERE business_id = %s AND status='Active'",
                (bid,),
            )[0]["count"]
...
def api_alerts_list():
    ...
    try:
        rows = execute_read_query_params(
            "SELECT alert_id, message, severity, status, created_at FROM alerts WHERE business_id = %s ORDER BY created_at DESC LIMIT 50",
            (bid,),
        )
        for r in rows:
...
def api_sales_target():
    ...
    if not bid:
        return jsonify(
            {"current_revenue": 0, "target_revenue": 100000, "percentage": 0}
        )
    try:
        rows = execute_read_query_params(
            """
        ...
        return jsonify(
            {"current_revenue": current, "target_revenue": target, "percentage": pct}
        )
    except Exception as exc:
        return internal_error_response(exc)
...
def api_alerts_by_severity():
    ...
    try:
        rows = execute_read_query_params(
            "SELECT severity, COUNT(*) AS cnt FROM alerts WHERE business_id = %s AND status='Active' GROUP BY severity",
            (bid,),
        )
        return jsonify(
            {
...
def api_top_products():
    ...
    try:
        rows = execute_read_query_params(
            "SELECT product_name, stock_quantity, selling_price, cost_price FROM products WHERE business_id = %s ORDER BY stock_quantity DESC LIMIT 10",
            (bid,),
        )
        margin_amount = [
        ...
        margin_pct = [
            round(
                ((r["selling_price"] or 0) - (r["cost_price"] or 0))
                / (r["selling_price"] or 1)
                * 100,
                1,
            )
            if r["selling_price"]
