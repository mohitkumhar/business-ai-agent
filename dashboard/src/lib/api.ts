...
export const api = {
  getSummary: async (period: string): Promise<DashboardSummary> => {
    const res = await fetch(`/api/dashboard/summary-sql?period=${period}`, {
      headers: getHeaders(),
    });
  ...
  getCategories: async () =>
    (
      await fetch(`/api/dashboard/categories`, { headers: getHeaders() })
    ).json(),
  ...
  exportDashboardCsv: async (period: string) => {
    ...
    const res = await fetch(`/api/dashboard/export-csv?${params.toString()}`, {
      headers: getHeaders(),
    });
